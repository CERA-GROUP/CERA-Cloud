...
class WMS (object):
    fields = ("bbox", "srs", "width", "height", "format", "layers", "styles", "transparent", "timestep")
    ...
    def __init__ (self, base, params, user=None, password=None):
        ...
        for key in self.fields:
            if params.has_key(key):
                ...
                self.params[key] = ""
        log = file("C:/ms4w/Apache/logs/cera_tilecache.log", "a+")
        print >> log, self.params
        log.close()
    ...
    def fetch (self):
        ...
        if True:
            try:
                response = self.client.open(urlrequest, timeout=10)
                data = response.read()
...
class LocalWMS (object):
    fields = ("bbox", "srs", "width", "height", "format", "layers", "styles", "timestep")
    defaultParams = {'version': '1.1.1', 'request': 'GetMap', 'service': 'WMS'}
    __slots__ = ("base", "params", "script", "data", "response")

    def __init__ (self, base, params, script):
        self.base = base
        self.script = script
        if self.base[-1] not in "?&":
            if "?" in self.base:
    ...
    def fetch (self):
        from CeraCache.Service import import_module

        object_module = import_module(self.script)
        main_function = getattr(object_module, "main")
        return main_function(self.params)
...
def seed (svc, layer, levels = (0, 5), bbox = None, padding = 0, force = False, reverse = False, delay = 0):
    ...
    for z in range(*levels):
        ...
        ztiles = int(math.ceil(float(topright[1] - bottomleft[1]) + 1) / metaSize[1] * \
                     math.ceil(float(topright[0] - bottomleft[0]) + 1) / metaSize[0])
        if reverse:
            startX = topright[0] + (1 * padding) # + metaSize[0]
            endX = bottomleft[0] - (1 * padding)
            stepX = -metaSize[0]
            startY = topright[1] + (1 * padding) # + metaSize[1]
            endY = bottomleft[1] - (1 * padding)
            stepY = -metaSize[1]
        else:
            startX = bottomleft[0] - (1 * padding)
            endX = topright[0] + (1 * padding) # + metaSize[0]
            stepX = metaSize[0]
            if stepX == 1:
                endX += 1

            startY = bottomleft[1] - (1 * padding)
            endY = topright[1] + (1 * padding) # + metaSize[1]
            stepY = metaSize[1]
            if stepY == 1:
                endY += 1
        ...
        for y in range(startY, endY, stepY):
            for x in range(startX, endX, stepX):
                ...
                if layer.isMetaTileLayer():
                    metatile = layer.getMetaTile(tile)
                    bounds = metatile.bounds()
                else:
                    bounds = tile.bounds()
        ...
        print "Elapsed time (layer: %d): %.3f [s]" % (z, (time.time() - start_layer + .0001))
...
def main (argv = None):
    ...
    parser = OptionParser(usage=usage, version="%prog $Id: Client.py 5647 2015-09-30 13:46:19Z cjesch $")
