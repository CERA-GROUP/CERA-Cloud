...
from CeraCache.Service import Request, Capabilities, TileCacheException
import CeraCache.Layer as Layer

class WMS (Request):
    def parse (self, fields, path, host):
        param = {}
        for key in ['bbox', 'layers', 'request', 'version', 'transparent']:
    ...
    def getMap (self, param):
        ...
        tiles =  []
        for name in layers:
            # skip any tiles out of bbox
            layer = self.getLayer(name)
            if param.has_key('transparent'):
                layer.transparent = param['transparent']
            tile = layer.getTile(bbox)
            if not tile:
#                if layer.debug:
#                    sys.stderr.write(
#                        "Skipping layer (outside of bbox?): %s, layer: %s\n" % (bbox, layer.name) )
                continue
            tiles.append(tile)
        ...
        if len(tiles) > 1:
            ...
            raise TileCacheException(
                "No tile found for layers %s from (%s), request outside of bounding box?"
                    % (str(layers), bbox))
        return tiles[0]
